...
export default function Save( { attributes } ) {
	...
	const contentHoverCss = backgroundHoverToCSS( uniqueId, contentBackground, ' .nx-tabs-content-wrapper' );
	...
	return (
		<div { ...blockProps }>
			<div className={ `nexus-tabs nx-tabs-${ tabLayout ?? 'top' }${ cssClasses ? ' ' + cssClasses : '' }` } data-activation={ tabActivation ?? 'click' } data-default-open={ defaultOpen ?? 0 }>
				...
				<div className="nx-tabs-content-wrapper" style={ { ...backgroundToStyle( contentBackground ), border: contentBorder?.style && contentBorder?.style !== 'none' ? `${ contentBorder.width ?? '1px' } ${ contentBorder.style } ${ contentBorder.color ?? '#E5E7EB' }` : undefined } }>
					{ contentHoverCss && <style dangerouslySetInnerHTML={ { __html: contentHoverCss } } /> }
					<InnerBlocks.Content />
				</div>
			</div>
		</div>
